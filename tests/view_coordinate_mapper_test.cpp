...
public:
    ModelIndex index(int r, int c, const ModelIndex &p = ModelIndex()) const override {
        return createIndex(r, c, reinterpret_cast<void *>(static_cast<uintptr_t>((r * 100) + c + 1)));
    }
...
TEST_F(ViewCoordinateMapperTest, ResetClearsActiveCountersWithoutDroppingCapacity) {
    ...
    // The counter is zeroed, so looking up the exact same coordinate must now fail
...
TEST_F(ViewCoordinateMapperTest, FindIndexAtCorrectlyIdentifiesContainingBoxes) {
    ViewCoordinateMapper mapper;
...
TEST_F(ViewCoordinateMapperTest, AddressesRemainCompletelyStableAcrossChunkSpillovers) {
    ViewCoordinateMapper mapper;
    ...
    // 2. Force memory allocation spillover by registering more items than CHUNK_SIZE (64)
    // This pushes registration through Chunk 1, Chunk 2, and into Chunk 3.
    for (int i = 0; i < 200; ++i) {
        ModelIndex filler_idx = model.index(i % 10, 0);
        mapper.registerCell(filler_idx);
    }

    // 3. Extract the address of our primary box again after massive allocations wrapped up
    ...
    // Any shift indicates vector reallocations occurred, which breaks ftxui::reflect contracts!
...
TEST_F(ViewCoordinateMapperTest, FirstRegisteredBoxWinsInOverlappingScenarios) {
    ...
    // A coordinate click hitting both blocks should prioritize the earliest registration entry

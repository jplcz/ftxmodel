...
class AbstractItemModel {
 public:
  ...
  /**
   * @brief Checks if the model has more data available for a specific parent
   * node.
   * This method is queried by views or proxy models before expanding a branch
   * to determine if an explicit data retrieval pass (@ref fetchMore) is
   * required. It allows the model to support lazy-loading or incremental
   * fetching of large or remote hierarchical datasets.
   * @note A return value of @c true implies that @rowCount(parent) currently
   * returns fewer items than are logically available on the disk, database, or
   * server.
   * @param parent The ModelIndex of the parent branch being evaluated. An
   * invalid index represents the root level of the model topology.
   * @return @c true if there is additional structural data available to be
   * fetched under the specified parent; @c false if all data is already loaded
   * or if the node is a leaf.
   * @sa fetchMore, hasChildren
   */
  virtual bool canFetchMore(const ModelIndex& parent) const {
    std::ignore = parent;
    return false;
  }

  /**
   * @brief Fetches additional available rows for the given parent node.
   * * This method performs the actual on-demand data population routine.
   * Implementations should retrieve the underlying dataset (e.g., executing an
   * SQL query, hitting a network REST gateway, or reading a local file
   * sub-tree) and inject the newly compiled indices into the model matrix
   * storage.
   * @attention Any implementation that inserts rows into the data structure
   * **must** wrap the insertion inside a @ref beginInsertRows() and @ref
   * endInsertRows() synchronization handshake block to safely alert attached
   * views to resize viewport matrices and scrollbar bounds.
   * @note For long-running network operations, it is highly recommended to
   * perform the actual data fetch asynchronously on a background worker thread,
   * updating internal loading state nodes, and finalizing the row injection on
   * the primary user interface thread. Model classes themselves are not
   * thread-safe.
   * @param parent The %ModelIndex of the target parent branch to populate
   * with children.
   * @sa canFetchMore, beginInsertRows, endInsertRows
   */
  virtual void fetchMore(const ModelIndex& parent) { std::ignore = parent; }

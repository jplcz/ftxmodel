...
class ScrollableTableView : public AbstractGridLikeItemView {
 ...
 public:
  ...
  bool OnEvent(ftxui::Event event) override {
    ...
    // If the target cursor row approaches within 5 rows of the current total,
    // check if the model has more data available, and fetch it instantly.
    constexpr int look_ahead_buffer = 5;
    if (r >= rowCount() - look_ahead_buffer) {
      ModelIndex root_parent =
          ModelIndex();  // Flat list root parent is invalid

      if (model()->canFetchMore(root_parent)) {
        model()->fetchMore(root_parent);
        // NOTE: model calls beginInsertRows -> updates vector -> calls
        // endInsertRows. Because endInsertRows is connected to this->update(),
        // the view recalculates rowCount() instantly before the remaining
        // execution block evaluates!
      }
    }

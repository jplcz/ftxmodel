...
class ConcatProxyModel : public AbstractItemModel {
 ...
 public:
  ...
  bool canFetchMore(const ModelIndex& proxyParent) const override {
    // If parent is invalid, we check if *any* of our root sub-models can fetch
    // more
    if (!proxyParent.isValid()) {
      for (const auto& srcModel : m_models) {
        if (srcModel->canFetchMore(ModelIndex())) {
          return true;
        }
      }
      return false;
    ...
    const auto resolved = mapFromProxy(proxyParent);
    if (!resolved.second) {
      return false;
    }

    return resolved.second->canFetchMore(resolved.first);
  }

  void fetchMore(const ModelIndex& proxyParent) override {
    if (!proxyParent.isValid()) {
      // Trigger fetchMore on the first root sub-model that demands it
      for (const auto& srcModel : m_models) {
        if (srcModel->canFetchMore(ModelIndex())) {
          srcModel->fetchMore(ModelIndex());
          return;
        }
      }
      return;
    ...
    resolved.second->fetchMore(resolved.first);
  }

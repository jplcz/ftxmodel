...
class FlattenTreeProxyModel : public AbstractProxyModel {
 public:
  ...
  ModelIndex mapToSource(const ModelIndex& child) const override {
    ...
    const ModelIndex cachedSrcIndex =
        impl_->flat_lookup_cache[static_cast<size_t>(child.row())];

    return sourceModel()->index(cachedSrcIndex.row(), child.column(),
                                cachedSrcIndex.parent());
  }

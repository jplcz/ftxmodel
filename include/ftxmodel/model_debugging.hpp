...
inline void dumpModelTreeHelper(const AbstractItemModel& model,
  ...
  for (int r = 0; r < rows; ++r) {
    ...
    // 2. Fetch the primary anchor column (Column 0 - typically the node Key)
    ModelIndex primaryIdx = model.index(r, 0, parent);
    std::string keyText = "[Invalid Index]";

    if (primaryIdx.isValid()) {
      auto dataVal = model.data(primaryIdx, ItemRole::DisplayRole);
      if (dataVal.has_value()) {
        keyText = std::any_cast<std::string>(dataVal);
      }
    }

    // 3. Collect and format any supplementary metadata columns (Type, Value,
    // etc.)
    std::string metadataColumns = "";
    for (int c = 1; c < cols; ++c) {
      ModelIndex colIdx = model.index(r, c, parent);
      if (colIdx.isValid()) {
        auto colData = model.data(colIdx, ItemRole::DisplayRole);
        if (colData.has_value()) {
          metadataColumns += " | " + std::any_cast<std::string>(colData);
        }
      }
    }

    // 4. Print the current line to the stream complete with indentation rails
    ss << prefix << marker << keyText << metadataColumns << "\n";

    // 5. If this node has sub-children, recursively branch deeper into the
    // matrix

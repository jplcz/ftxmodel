...
namespace ftxmodel {
  ...
  public:
    ...
    /**
     * @brief Registers a data cell and provides a completely memory-stable Box handle.
     * Addresses returned here are guaranteed to remain unchanged even if additional
     * capacity buckets are allocated during the pass.
     */
    ftxui::Box &registerCell(const ModelIndex &index) {
      ...
      // Allocate an incremental stable bucket block only if we spill out of current pools
      ...
      m_active_count++;
      return entry.box; // Safe reference return! No reallocations can break this address.
    }

    /**
     * @brief Evaluates terminal intersection bounds across stable memory chunks.
     */
    [[nodiscard]] std::optional<ModelIndex> findIndexAt(const int mouse_x, const int mouse_y) const noexcept {
